...
const Merch = () => {
  ...
  const checkAllImagesLoaded = (merchData) => {
    return new Promise((resolve, reject) => {
      ...
      const totalImages = merchData.length

      merchData.forEach((item) => {
        const image = new Image()
        image.src = `https://raw.githubusercontent.com/Evolution-X/www_gitres/main/merch/items/${item.image}.png?raw=true`
        image.onload = () => {
          loadedImagesCount++
        ...
        image.onerror = () => {
          reject(new Error(`Failed to load image for item ${item.name}`))
        }
      })
    })
  }
        ...
        {merchData.map((item, index) => (
          ...
          >
            <img
              ...
              alt={item.name}
              src={`https://raw.githubusercontent.com/Evolution-X/www_gitres/main/merch/items/${item.image}.png?raw=true`}
            />
